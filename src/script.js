...
let giantX = 7;
let giantZ = 7;
let giantRotation = Math.PI / 2;
let skeletons = [];

const loader = new GLTFLoader();
const loseDistance = 2;

loader.load("textures/models/Cube_World/Giant.glb", (gltf) => {
  ...
  function handleKeyDown(event) {
    ...
    giant.position.set(giantX, 0, giantZ);
    giant.rotation.y = giantRotation;

    const distanceToDoor = giant.position.distanceTo(door.position);

    if (distanceToDoor < 1.0) {
    ...
    for (let i = 0; i < skeletons.length; i++) {
      ...
      const distance = giant.position.distanceTo(skeleton.position);

      if (distance < loseDistance) {
        console.log("Partie perdue !");
        window.alert("Partie perdue !");
      }
    }
  }
});
...
for (let i = 0; i < 8; i++) {
  ...
  loader.load("textures/models/Cube_World/Skeleton.glb", (gltf) =>
    addSkeleton(gltf, new THREE.Vector3(x, 0, z))
  );
}
...
for (let z = -9; z <= 9; z += 0.6) {
  const newPosition = new THREE.Vector3(9.9, 0.3, z);
...
for (let x = 9; x >= -9; x -= 0.6) {
  const newPosition = new THREE.Vector3(x, 0.3, 9.9);
  rotate = true;
  positions.push(newPosition);
}
for (let x = 9; x >= -9; x -= 0.6) {
...
for (let i = 0; i < positions.length; i++) {
  loader.load("textures/models/Cube_World/Fence_Center.glb", (gltf) =>
    addFenceCenter(gltf, positions[i])
  );
  if (rotate == true) {
    // fenceCenter.rotation.y = Math.PI / 2;
  }
...
ghost1.shadow.camera.far = ghost2.castShadow = true;
